...
export function tmpFilePath(prefix = "test") {
  const ts = Date.now();
  const rand = Math.random().toString(36).slice(2, 8);
  return join(tmpDir, `${prefix}-${ts}-${rand}.json`);
}
...
export async function copyDataFileToTmp(prefix = "test") {
  await ensureTmpDir();
...
export async function seedTmpFile(seedData, prefix = "test") {
  await ensureTmpDir();
  const target = tmpFilePath(prefix);
...
export async function startServer(dbPath, customSeed = null, port = null) {
  await ensureTmpDir();
  if (customSeed) {
